...
void configWebServerRegisterRoutes() {
    ...
    webServer.on("/setting", []() {
        ...
        if (qsid.length() > 0 && qpass.length() > 0) {
            eraseData(0, 96)
            saveDataAsString(0, qsid);
            saveDataAsString(32, qpass);
            dataCommit();
 ...
 *
 */
String readDataAsString(int from, int to) {
    String data = "";
 ...
 *
 */
void eraseData(int from, int to) {
    for (int i = from; i < to; ++i) {
        EEPROM.write(i, 0);
    }
}
 ...
 *
 */
void saveDataAsString(int offset, String value) {
